...
- **`assets/cv.pdf`** — кнопка CV в сайдбаре и футере ведёт на этот файл, его нужно положить в репозиторий.
- **Ссылки на соцсети** — у кнопок `LinkedIn`, `Behance` и `Telegram` в `index.html` сейчас стоит
  заглушка `href="#"`. Пришлите адреса — подставлю.
- **Английская версия** — переключатель `RU` пока только меняет надпись, второго языка в макете нет.
