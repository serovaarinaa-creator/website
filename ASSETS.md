...
- за 800px до появления блока скрипт переключает ролик на `preload="auto"` и
  вызывает `load()`, чтобы он успел набрать буфер заранее;
- играет всегда ровно один ролик — тот, которого видно больше всех по площади;
  остальные стоят на паузе. На стоимость прокрутки это, по замерам в WebKit,
  не влияет (25 мс против 25,5 мс), но экономит процессор и батарею;
