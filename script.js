...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (new URLSearchParams(location.search).get("video") === "off") {
    ...
    /* У роликов ниже первого экрана адрес лежит в разметке не в src, а в
       data-src, и подставляется скриптом, когда до блока остаётся меньше
       экрана прокрутки.

       Так сделано из-за Safari. Ниже по файлу мы гасим за экраном флаг
       автозапуска, и одновременное декодирование это ограничивает — но не
       загрузку: у ролика с autoplay и готовым src Safari начинает качать
       файл сразу, ещё до того, как скрипт успеет что-то погасить. На главной
       это восемь файлов и девять мегабайт разом, которые лезут в ту же
       очередь, что и картинки первого экрана, — отсюда и ощущение, что
       страница долго собирается.

       Пустой src эту загрузку не начинает вовсе: на месте ролика стоит кадр
       из poster, а файл подъезжает уже под прокрутку. Autoplay при этом
       остаётся в разметке — заводит ролик по-прежнему браузер, а не play(),
       так что кнопка Safari поверх видео не появляется (почему это важно,
       расписано ниже).

       А вот ролики первого экрана, наоборот, обязаны держать адрес прямо в
       src. Их всё равно нужно показать сразу, а разметочный src браузер
       находит предсказывающим разбором ещё до того, как дойдёт до конца
       body и запустит этот файл. Обложка кейса из-за отложенной загрузки
       заводилась заметно позже, чем раньше, — так что откладывать имеет
       смысл ровно то, что не видно. Какие именно ролики считать первым
       экраном, решено в разметке: у них src, у остальных data-src. */
    const attach = (video) => {
    ...
    const warmer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          ...
          /* Здесь ролик и получает свой адрес — за 600px до появления блока.
             Порядок важен: сначала src, потом preload, иначе поднятый preload
             ещё нечему применяться. */
          /* Присвоение src, как и load(), возвращает элементу флаг
             автозапуска, а с ним и autoplay из разметки — иначе ролик завёлся
             бы прямо тут, за 600px до экрана, мимо ограничения на
             одновременное декодирование. Поэтому сразу же гасим флаг тем же
             pause(), что и при загрузке страницы: качать — качай, играть
             будешь от наблюдателя ниже. Загрузку pause() не отменяет, ей
             распоряжается preload. */
          if (attach(video)) disarm(video);
    ...
    /* Ролики, у которых флаг автозапуска погасили мы сами своим pause().
       Только им нужен load(), и знать это важно: load() не просто взводит
       флаг, он ещё и сбрасывает элемент, обрывая уже идущую загрузку.

       Раньше load() звался всему, что оказалось на экране, — в том числе
       ролику первого экрана, который никто не гасил и который браузер уже
       качал сам, начав ещё при разборе разметки. Загрузка обрывалась и
       начиналась заново, отсюда и пауза перед первым кадром на обложке. */
    const disarmed = new WeakSet();
    ...
    const nudge = (video) => {
      /* Подстраховка на случай, когда до ролика долистали быстрее, чем
         отработал колбэк warmer'а, — и на первый экран, который проверяется
         напрямую по геометрии, вообще не дожидаясь наблюдателей. */
      attach(video);
      ...
      /* Флаг автозапуска гасили мы — возвращаем его load()'ом. Если не гасили,
         ролик заведётся сам: у первого экрана флаг с разбора разметки не
         снимался, а присвоение src в attach() взводит его заново. В обоих
         случаях load() только помешал бы. */
      if (disarmed.has(video)) {
        disarmed.delete(video);
        video.load();
      }
      ...
      /* Если автозапуск запрещён (Safari так делает в энергосбережении и когда
         ...
         этом не сообщит — промиса тут нет. Поэтому проверяем следом и уже
         тогда пробуем скриптом: там отказ виден и ролик уходит в постер. */
      armRetry(video);
    };

    /* Отличить запрет автозапуска от медленной сети можно только по данным:
       браузер не заводит ролик и в том, и в другом случае, и молчит в обоих.

       Разница в readyState. Если данных на проигрывание ещё не набралось,
       это не запрет, а загрузка — и звать тут play() прямо вредно: поверх
       заведённого скриптом ролика Safari рисует свою круглую кнопку, которую
       не убрать стилями. Из-за этого кнопки и вылезали на медленном
       соединении. Поэтому пока ролик грузится, мы просто ждём дальше, а
       скриптовый запуск оставляем на случай, когда данные уже есть, а
       воспроизведение всё равно не началось — вот это и есть запрет. */
    const RETRY_STEP = 1500;
    const RETRY_LIMIT = 12;

    const armRetry = (video, left = RETRY_LIMIT) => {
      ...
      retries.set(
        ...
        setTimeout(() => {
          ...
          /* HAVE_FUTURE_DATA: данных хватает, чтобы играть дальше. */
          if (video.readyState < 3 && left > 0) {
            armRetry(video, left - 1);
            return;
          }
          scriptedPlay(video);
        }, RETRY_STEP)
