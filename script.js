document.addEventListener("DOMContentLoaded", () => {
  ...
  if (new URLSearchParams(location.search).get("video") === "off") {
    ...
    const nudge = (video) => {
      ...
      if (!play) return;
      play.catch((err) => {
        /* Подменяем постером только при настоящем запрете автозапуска
           (NotAllowedError) — ради него вся эта ветка и написана.

           Раньше подменялся любой отказ, и это выходило боком: play() ждёт
           данные, ролик за это время уходит с экрана, наш же pause() рвёт
           промис с AbortError — и ролик навсегда оставался картинкой, хотя
           браузер ничего не запрещал. То же самое с ошибками загрузки: их
           лечит следующий canplay, а не подмена. Пока данных нет, на месте
           видео и так стоит кадр из атрибута poster, так что терять нечего. */
