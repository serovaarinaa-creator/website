...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (new URLSearchParams(location.search).get("video") === "off") {
    ...
    /* Адрес ролика лежит в разметке не в src, а в data-src, и подставляется
       скриптом, когда до блока остаётся меньше экрана прокрутки.
    ...
    const nudge = (video) => {
      /* Подстраховка на случай, когда до ролика долистали быстрее, чем
         ...
         Если src подставился прямо сейчас, load() ниже уже не нужен: его
         работу — вернуть флаг автозапуска и начать загрузку — присвоение src
         делает само, а лишний вызов оборвал бы только что начатую загрузку. */
      const justAttached = attach(video);
